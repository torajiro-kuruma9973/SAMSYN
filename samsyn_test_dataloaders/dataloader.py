...
class dataset_3d(Dataset):
    def __init__(self, args):
        ...
        test_data_paths = [name for name in os.listdir(args.data_root) if os.path.isfile(os.path.join(args.data_root, name))]
    
        data_path_list = []
        
        for i in range(len(test_data_paths)):
            img_name = args.data_root + test_data_paths[i]
...
def get_dataset_3d(args):
    test_data = dataset_3d(args)
    train_sampler = DistributedSampler(test_data, shuffle=False) if args.dist else None
